...
function unirseAlJuego() {
    fetch("http://192.168.1.6:8080/unirse").then(function (res) {
        if (res.ok) {
...
function seleccionarMokepon(mascotaJugador) { fetch(`http://192.168.1.6:8080/mokepon/${jugadorId}`, { method: "post", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mokepon: mascotaJugador }) }) }
...
function enviarAtaques() {
    fetch(`http://192.168.1.6:8080/mokepon/${jugadorId}/ataques`, { method: "post", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ataques: ataqueJugador }) })
    intervalo = setInterval(obtenerAtaques, 50)
}
function obtenerAtaques() {
    fetch(`http://192.168.1.6:8080/mokepon/${enemigoId}/ataques`).then(function (res) {
        if (res.ok) {
...
function ataqueAleatorioEnemigo() {
    console.log('Ataques enemigo', ataquesMokeponEnemigo); let ataqueAleatorio = aleatorio(0, ataquesMokeponEnemigo.length - 1)
...
function enviarPosicion(x, y) {
    fetch(`http://192.168.1.6:8080/mokepon/${jugadorId}/posicion`, { method: "post", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ x, y }) }).then(function (res) {
        if (res.ok) {
